...
new p5(p5 => {
  ...
  const clamp_road = (x) => (x < min_buffer) ? min_buffer : (x > Math.round(WIDTH / 2) - min_buffer) ? Math.round(WIDTH / 2) - min_buffer : x
  ...
  const update_path = () => {
    ...
    if (choice) {
      modifier = -1
      turn = ((current_pos_x - min_turn) > min_buffer)
        ? getRandomInt(min_turn, current_pos_x - 5)
        : current_pos_x - min_buffer
    } else {
      modifier = 1
      turn = ((Math.floor(WIDTH / 2) - current_pos_x - min_buffer) > min_turn)
        ? getRandomInt(min_turn, (Math.floor(WIDTH / 2) - current_pos_x - min_buffer))
    ...
    for (let y = block_size; y < height; y += block_size) {
      ...
      queue.push([clamp_road(current_pos_x + x), 0, Math.round(WIDTH / 2), block_size])
    }
  ...
  p5.draw = () => {
    ...
    let collision = false
    for (let i = 0; i < 16; i++) {
      if (new_pos > road[75 + i][0] && new_pos + 16 < road[75 + i][1] + road[75 + i][3]) {
        collision = true
      }
    }
    if (collision) {
