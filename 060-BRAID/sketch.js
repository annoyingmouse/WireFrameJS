new p5((p5) => {
  ...
  const update = () => {
    ...
    if (p5.keyIsDown(8)) {
      ...
      const rgbtop = collisionMap
        .get(Math.round(tim.x - levelx), Math.round(tim.y + 30))
        .slice(0, 3);
      const rgbbottom = collisionMap
        .get(Math.round(tim.x - levelx), Math.round(tim.y + 50))
        .slice(0, 3);
      tim.ystore = tim.y;
      tim.xstore = tim.x;
      if (p5.keyIsDown(37)) {
  ...
  const doGravity = () => {
    const rgb = collisionMap
      .get(Math.round(tim.x - levelx), Math.round(tim.y + 25 + 50))
      .slice(0, 3);
  ...
  p5.keyPressed = () => {
    if (p5.keyCode === 32) {
      const rgb = collisionMap
        .get(Math.round(tim.x - levelx), Math.round(tim.y + 100))
        .slice(0, 3);
      if (equals(rgb, [0, 0, 0])) {
        tim.frame = 6;
        tim.jumping = 12;
  ...
  const checkMove = (xinc) => {
    const rgb = collisionMap
      .get(Math.round(tim.x - levelx + xinc), Math.round(tim.y + 50))
      .slice(0, 3);
