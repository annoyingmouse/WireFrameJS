new p5((p5) => {
  ...
  const drawChars = () => {
    ...
    link.image =
      link.images[link.dir * 2 + 1 + Math.floor(link.frame / 10) - 1];
    ...
    monsters.forEach((monster) => {
      if (onScreen(monster.x, monster.y) && monster.state > 0) {
        ...
        if (monster.state === 10) {
          monster.image =
            monster.images[
              monster.dir * 2 + 1 + Math.floor(monster.frame / 10) - 1
            ];
        }
  ...
  const moveChars = () => {
    ...
    if (link.movex || link.movey) {
      ...
      if (
        equals(
          map.image.get(link.testx + map.x, link.testy + map.y).slice(0, 3),
          [0, 0, 0],
        )
      ) {
        link.x += link.movex * 2;
      ...
      if (onScreen(monster.x, monster.y) && monster.state === 10) {
        ...
        if (monster.movex || monster.movey) {
          ...
          if (
            equals(
              map.image
                .get(monster.testx + map.x, monster.testy + map.y)
                .slice(0, 3),
              [0, 0, 0],
            )
          ) {
