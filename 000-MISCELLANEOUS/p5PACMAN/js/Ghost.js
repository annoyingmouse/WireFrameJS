class Ghost {
	...
	draw() {
		...
		if (pacman.frames) {
			...
			fill("#dedeff");
			beginShape();
			this.LeftEye.forEach((p) =>
				vertex(
					this.x + p[0] + this.Eyes[this.direction][0],
					this.y + p[1] + this.Eyes[this.direction][1],
				),
			);
			endShape(CLOSE);
			beginShape();
			this.RightEye.forEach((p) =>
				vertex(
					this.x + p[0] + this.Eyes[this.direction][0],
					this.y + p[1] + this.Eyes[this.direction][1],
				),
			...
			fill("#2121ff");
			rect(
				this.x + 1 + this.Eyes[this.direction][2],
				this.y - 9 + this.Eyes[this.direction][3],
				2,
				2,
			);
			rect(
				this.x + 7 + this.Eyes[this.direction][2],
				this.y - 9 + this.Eyes[this.direction][3],
				2,
				2,
			);
	...
	update() {
		...
		if (!(this.x % halfScale) && !(this.y % halfScale)) {
			...
			const portalIndex = portals.findIndex(
				(el) => el[0] === this.x + halfScale && el[1] === this.y - halfScale,
			);
			if (!!~portalIndex) {
				...
				if (this.intention.length) {
					...
					const possibleDirections = field.ask(
						this.x + halfScale,
						this.y - halfScale,
					).allowed;
					...
					switch (this.direction) {
						case "RIGHT":
							if (!!~possibleDirections.indexOf("LEFT")) {
								possibleDirections.splice(
									possibleDirections.indexOf("LEFT"),
									1,
								);
							}
							break;
						case "LEFT":
							if (!!~possibleDirections.indexOf("RIGHT")) {
								possibleDirections.splice(
									possibleDirections.indexOf("RIGHT"),
									1,
								);
							}
							break;
						case "UP":
							if (!!~possibleDirections.indexOf("DOWN")) {
								possibleDirections.splice(
									possibleDirections.indexOf("DOWN"),
									1,
								);
							}
							break;
