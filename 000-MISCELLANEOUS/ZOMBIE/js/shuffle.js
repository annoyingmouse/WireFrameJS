...
(async () => {
  ...
  const processMyArray = async (manifestArray) => {
    ...
    for (const manifest of manifestArray) {
      const json = await fetch(manifest)
        ...
        .then(async (json) => {
          json.images = await loadImages(
            Array.from({ length: json.imageNumber }, (_, i) => i + 1).map(
              (n) =>
                `${json.location}frame_${n.toString().padStart(2, "0")}.png`,
            ),
          );
  ...
  for (const prop in zombiesNum) {
    if (zombiesNum.hasOwnProperty(prop)) {
      ...
      const zombie =
        zombieObjectArray[Math.floor(Math.random() * zombieObjectArray.length)];
      zombies.push(
        new Zombie(
          zombie.images,
          ...
          zombie.steps,
          zombie.lurch,
          Math.floor(Math.random() * zombie.images.length),
        ),
      );
    }
  }
