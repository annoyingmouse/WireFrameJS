...
// const input = `[.#.#] (0,2,3) (1,3) {11,10,11,21}
...
(() => {
  const rows = input.split("\n").map((row) => {
    ...
    return {
      ...
      joltage: input[input.length - 1]
        .substring(1, input[input.length - 1].length - 1)
        .split(",")
        .map((e) => Number.parseInt(e, 10)),
      buttons: input.slice(1, -1).map((b) =>
        b
  ...
  for (const row of rows) {
    ...
    for (
      let permutation = 1;
      permutation <= 1 << row.buttons.length;
      permutation++
    ) {
      const filteredButtons = row.buttons.filter((button, index) => {
      ...
      if (attempt === row.sequence) {
        rowSolution = Math.min(filteredButtons.length, rowSolution);
      }
    }
    runningTotal1 += rowSolution;
  }
