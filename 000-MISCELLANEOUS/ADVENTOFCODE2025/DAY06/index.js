(() => {
  ...
  const raw1 = input
    .slice(1)
    .split("\n")
    .map((e) => e.trim().split(/\s+/g));
  const rotated = rotateMatrix90C(raw1);
  for (const row of rotated) {
    const tempInput1 = row.slice(1).map((e) => Number(e));
    if (row[0] === "+") {
      runningTotal1 += tempInput1.reduce((a, b) => a + b);
    }
    if (row[0] === "*") {
      runningTotal1 += tempInput1.reduce((a, b) => a * b);
    }
  ...
  // console.assert(
  //   runningTotal1 === 4277556,
  //   "There's something wrong with the sum",
  // );
  console.log("Part 1:", runningTotal1);
  const rawRows = input.slice(1).split("\n");
  const rawRowsMapping = rawRows[rawRows.length - 1]
    .split(/([\*\+]\s+)/g)
    .reduce((a, c) => (c ? a.concat([c.length]) : a), []);
  const rawRowsOperator = rawRows[rawRows.length - 1]
    .split(/([\*\+]\s+)/g)
    .reduce((a, c) => (c ? a.concat([c.trim()]) : a), []);
  rawRows.pop(); // remove the last row, now that we've got the mapping!
  // console.log(rawRowsMapping);
  // console.log(`Maximum chunk size: ${Math.max(...rawRowsMapping)}`);
  // console.log(`Minimum chunk size: ${Math.min(...rawRowsMapping)}`);
  const columns = [];
  let runningStart = 0;
  for (const rawRowMapping of rawRowsMapping) {
    const column = [];
    // console.log(runningStart, rawRowMapping);
    for (const rawRow of rawRows) {
      // console.log(rawRow);
      column.push(rawRow.slice(runningStart, runningStart + rawRowMapping));
    }
    runningStart += rawRowMapping;
    columns.push(column);
  }
  const convertNumbers = (arr) => {
    ...
    for (let col = 0; col < maxLen; col++) {
      ...
      // read top → bottom across rows
      for (let row = 0; row < arr.length; row++) {
        const ch = arr[row][col];
        if (ch && ch !== " ") num += ch;
      }

      if (num.length > 0) cols.push(Number(num));
    }

    // reverse to match required output order
    return cols.reverse();
  };
  // console.log(columns)
  for (let i = 0; i < columns.length; i++) {
    // console.log(rawRowsOperator[i])
    // console.log(convertNumbers(columns[i]))
    if (rawRowsOperator[i] === "+") {
      runningTotal2 += convertNumbers(columns[i]).reduce((a, b) => a + b);
    }
    if (rawRowsOperator[i] === "*") {
      runningTotal2 += convertNumbers(columns[i]).reduce((a, b) => a * b);
    }
